...
% Modified slightly by Thomas Sibley in Nov 2018.
...
% Declarations:
\newdimen\barcodeX            % the module size X,
\newdimen\bcorr        % the bar correction (see bellow).
\newdimen\workdimen \newdimen\barcodeHeight   % internal variables
\newtoks\inputtext \newtoks\icode
\newcount\tempnum \newcount\chnum \newcount\chtotal
\newif\ifnext \newif\ifchar
...
% Implicit values:
\barcodeX=.33mm         % The X module width.
\bcorr=.020mm    % Bar reduction.
\barcodeHeight=1.5cm % The code height.
...
\def\makebars#1#2{\if0#2\let\next=\endcode\fi
  \workdimen=#1\barcodeX \advance\workdimen by-\bcorr \vrule width\workdimen
  \workdimen=#2\barcodeX \advance\workdimen by \bcorr \kern\workdimen
  \next}

% The begin and end of completed \hbox:
\def\begcode{\hbox\bgroup\vrule height\barcodeHeight width0pt}
\def\endcode{\egroup}
...
multiples of basic dimension: so called X module size (see \barcodeX in macro).
